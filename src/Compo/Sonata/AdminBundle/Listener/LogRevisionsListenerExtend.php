...
namespace Compo\Sonata\AdminBundle\Listener;
...
/**
 * Class LogRevisionsListenerExtend.
 */
class LogRevisionsListenerExtend implements EventSubscriber
{
    /**
     * @var array
     */
    public $auditedEntitiesIgnoreColumns = [];
    ...
    /**
     * LogRevisionsListenerExtend constructor.
     *
     ...
     * @return array
     */
    public function getAuditedEntitiesIgnoreColumns(): array
    {
        return $this->auditedEntitiesIgnoreColumns;
    }

    /**
     * @param array $auditedEntitiesIgnoreColumns
     */
    public function setAuditedEntitiesIgnoreColumns(array $auditedEntitiesIgnoreColumns): void
    {
        foreach ($auditedEntitiesIgnoreColumns as $item) {
            $this->auditedEntitiesIgnoreColumns[$item['entity']] = $item['columns'];
        }
    }
    ...
    public function postFlush(PostFlushEventArgs $eventArgs)
    {
        ...
        foreach ($this->extraUpdates as $entity) {
            ...
            /** @var array $updateDataColumns */
            $updateDataColumns = $updateData[$meta->table['name']];

            foreach ($updateDataColumns as $column => $value) {
                ...
                foreach ($this->config->getGlobalIgnoreColumns() as $columnName) {
                    if ($field === $columnName) {
                        $isIgnore = true;
                    }
                }

                if (isset($this->auditedEntitiesIgnoreColumns[$className])) {
                    /** @var array $auditedEntitiesIgnoreColumns */
                    $auditedEntitiesIgnoreColumns = $this->auditedEntitiesIgnoreColumns[$className];

                    foreach ($auditedEntitiesIgnoreColumns as $columnName) {
                        if ($field === $columnName) {
                            $isIgnore = true;
                        }
                    }
                ...
                if (\in_array($column, $meta->columnNames, true)) {
                    ...
                    foreach ($meta->associationMappings as $mapping) {
                        if (isset($mapping['joinColumns'])) {
                            /** @var array $joinColumns */
                            $joinColumns = $mapping['joinColumns'];

                            /** @var array $definition */
                            foreach ($joinColumns as $definition) {
    ...
    public function postUpdate(LifecycleEventArgs $eventArgs)
    {
        ...
        if (isset($this->auditedEntitiesIgnoreColumns[$class->name])) {
            /** @var array $auditedEntitiesIgnoreColumns */
            $auditedEntitiesIgnoreColumns = $this->auditedEntitiesIgnoreColumns[$class->name];

            foreach ($auditedEntitiesIgnoreColumns as $column) {
                if (isset($changeset[$column])) {
                    unset($changeset[$column]);
                }
            }
        ...
        if (!isset($this->insertRevisionSQL[$class->name])) {
            ...
            foreach ($class->associationMappings as $field => $assoc) {
                ...
                if (($assoc['type'] & ClassMetadata::TO_ONE) > 0 && $assoc['isOwningSide']) {
                    /** @var array $targetToSourceKeyColumns */
                    $targetToSourceKeyColumns = $assoc['targetToSourceKeyColumns'];

                    foreach ($targetToSourceKeyColumns as $sourceCol) {
    ...
    private function saveRevisionEntityData($class, $entityData, $revType)
    {
        ...
        foreach ($class->associationMappings as $field => $assoc) {
            ...
            $sourceToTargetKeyColumns = $assoc['sourceToTargetKeyColumns'];

            foreach ($sourceToTargetKeyColumns as $sourceColumn => $targetColumn) {
        ...
        } elseif ($class->name === $class->rootEntityName && $class->isInheritanceTypeJoined()) {
            $params[] = $entityData[$class->discriminatorColumn['name']];
            $types[] = $class->discriminatorColumn['type'];
        }

        if ($class->name !== $class->rootEntityName && $class->isInheritanceTypeJoined()) {
    ...
    private function prepareUpdateData($persister, $entity)
    {
        ...
        $versioned = $classMetadata->isVersioned;
        ...
        foreach ($uow->getEntityChangeSet($entity) as $field => $change) {
            if (null !== $versionField && $versionField === $field) {
                continue;
            }
            ...
            if (null !== $newVal && $uow->isScheduledForInsert($newVal)) {
                $newVal = null;
            }
